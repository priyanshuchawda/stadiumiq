...
- **Controls:**
  ...
  - Strict CSP disallows inline scripts (except hashed), restricts `connect-src`/`img-src`/`frame-ancestors`.
...
- **Controls:**
  - Per-IP + per-session token-bucket rate limiting on all AI routes → `429` + `Retry-After`.
...
## 4. Security Headers (configured in `next.config`)
