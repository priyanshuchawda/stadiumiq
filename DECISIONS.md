...
## Pending decisions (AI coder to resolve and record here)
- **ADR-009 (TODO): Error-handling strategy** — choose `Result<T,E>` returns vs typed thrown errors mapped centrally; apply consistently. Record the choice and rationale.
- Any library swap due to build incompatibility → add a new ADR with the closest maintained OSS replacement and why.
